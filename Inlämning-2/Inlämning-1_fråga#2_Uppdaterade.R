...
points_within_circle <-
  which(sqrt(x ^ 2 + y ^ 2) <= 1) #displays index of points within circle
points_outside_circle <-
  which(sqrt(x ^ 2 + y ^ 2) > 1) #shows index of points outside circle

#An approximate value of pi from the sampled value
pi_hat <-
  4 * (length(points_within_circle) / nr_of_iterations) #estimate of "pi
...
df_xy <- data.frame(x, y, sqrt(x ^ 2 + y ^ 2))
library(tidyverse)
...
library(ggplot2)
sc <- ggplot(df_xy_Region, aes(x, y, colour = Region)) +
